...
import CalculatorResultsTableRow from "./risk-doctor/calculator-results-table-row";
...
import SelectPairInput from "./forms/select-asset-input";
import { zodResolver } from "@hookform/resolvers/zod";
import { AccountCurrency, ExecutionType } from "@/types/calculator-types";
...
export default function RiskDoctorResults() {
  ...
  return (
    <div className="w-full space-y-8">
      <div className="w-full">
        {/* {Trade level simulation bar sl - Entry - Tp} */}
        <div className="w-full px-4">
          <ResizablePanelGroup direction="horizontal" className="h-[1px]">
            ...
            <ResizablePanel className="space-y-4 ">
              ...
              <div className="h-1 w-full bg-green-500 rounded-r-xl"> </div>
            </ResizablePanel>
          </ResizablePanelGroup>
        </div>
      </div>
      <Tabs defaultValue="scenerio" className="w-full rounded-xl m-0 p-0">
        <TabsList className="grid w-full grid-cols-2 bg-transparent m-0 items-center  px-4 ">
          <TabsTrigger
            key={"scenerio"}
            value={"scenerio"}
            className="data-[state=active]:bg-primary data-[state=active]:shadow-none  data-[state=active]:text-background text-primary rounded-full cursor-pointer w-full"
          >
          ...
          <Card className="bg-white shadow-none tableLine gap-0 py-2 px-4 ">
            {calculations.map((calc, index) => (
              <CalculatorResultsTableRow
                ...
                currentValues={{
                  exectutionType:
                    riskData?.exectutionType === "Sell" ? "Sell" : "Buy",
                  entry: entry ?? 0,
      ...
      <div className="flex justify-center w-full  pl-6 mx-auto">
        <Dialog>
          <form>
            <DialogTrigger asChild>
              <Button variant={"outline"} size={"sm"} className="">
                <RefreshCcwDot /> Recalculate Risk
              </Button>
            </DialogTrigger>
            <DialogContent className="w-full max-w-8xl border-0">
              <DialogHeader>
                <DialogTitle>Recalculate Risk</DialogTitle>
                <DialogDescription>
